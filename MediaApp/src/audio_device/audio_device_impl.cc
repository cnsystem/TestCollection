                                                 ...
                                                 const AudioLayer audioLayer)
{
    ...
    // Create the generic ref counted (platform independent) impleWEBRTC_ntation.
    ...
    // Create the platform-dependent impleWEBRTC_ntation.
...
WebRtc_Word32 AudioDeviceModuleImpl::CreatePlatformSpecificObjects()
{
    ...
    // Create the *Windows* impleWEBRTC_ntation of the Audio Device
    //
#if defined(_WIN32)
    ...
    {
        // create *Windows Wave Audio* impleWEBRTC_ntation
        ...
        )
    {
        ...
        {
            // create *Windows Core Audio* impleWEBRTC_ntation
    ...
    if (ptrAudioDevice != NULL)
    {
        // Create the Windows impleWEBRTC_ntation of the Device Utility.
    ...
    // Create the *Android OpenSLES* impleWEBRTC_ntation of the Audio Device
    //
#if defined(WEBRTC_ANDROID_OPENSLES)
    if (audioLayer == kPlatformDefaultAudio)
    {
        // Create *Android OpenELSE Audio* impleWEBRTC_ntation
    ...
    if (ptrAudioDevice != NULL)
    {
        // Create the Android impleWEBRTC_ntation of the Device Utility.
        ptrAudioDeviceUtility = new AudioDeviceUtilityAndroid(Id());
    }
    // END #if defined(WEBRTC_ANDROID_OPENSLES)

    // Create the *Android Java* impleWEBRTC_ntation of the Audio Device
    //
#elif defined(WEBRTC_ANDROID)
    if (audioLayer == kPlatformDefaultAudio)
    {
        // Create *Android JNI Audio* impleWEBRTC_ntation
    ...
    // Create the *Linux* impleWEBRTC_ntation of the Audio Device
    //
#elif defined(WEBRTC_LINUX)
    if ((audioLayer == kLinuxPulseAudio) || (audioLayer == kPlatformDefaultAudio))
    {
        ...
        {
            // create *Linux PulseAudio* impleWEBRTC_ntation
...
#endif
#if defined(LINUX_ALSA)
            // create *Linux ALSA Audio* impleWEBRTC_ntation
    ...
    if (ptrAudioDevice != NULL)
    {
        // Create the Linux impleWEBRTC_ntation of the Device Utility.
        // This class is independent of the selected audio layer
        // for Linux.
        //
    ...
    // Create the *iPhone* impleWEBRTC_ntation of the Audio Device
    //
#if defined(WEBRTC_IOS)
    if (audioLayer == kPlatformDefaultAudio)
    {
        // Create *iPhone Audio* impleWEBRTC_ntation
    ...
    if (ptrAudioDevice != NULL)
    {
        // Create the Mac impleWEBRTC_ntation of the Device Utility.
        ptrAudioDeviceUtility = new AudioDeviceUtilityIPhone(Id());
    }
    // END #if defined(WEBRTC_IOS)

    // Create the *Mac* impleWEBRTC_ntation of the Audio Device
    //
#elif defined(WEBRTC_MAC)
    if (audioLayer == kPlatformDefaultAudio)
    {
        // Create *Mac Audio* impleWEBRTC_ntation
    ...
    // Create the *Dummy* impleWEBRTC_ntation of the Audio Device
    // Available for all platforms
    //
    if (audioLayer == kDummyAudio)
    {
        // Create *Dummy Audio* impleWEBRTC_ntation
    ...
    if (ptrAudioDevice == NULL)
    {
        WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id, "unable to create the platform specific audio device impleWEBRTC_ntation");
        return -1;
    }
...
//  AttachAudioBuffer
//
//  Install "bridge" between the platform impleWEBRTC_tation and the generic
//  impleWEBRTC_ntation. The "child" shall set the native sampling rate and the
    ...
    char guid[kAdmMaxGuidSize])
{
    ...
    if (name == NULL)
    {
        _lastError = kAdWEBRTC_rrArgument;
        return -1;
    }
...
WebRtc_Word32 AudioDeviceModuleImpl::SetPlayoutBuffer(const BufferType type, WebRtc_UWord16 sizeMS)
{
    ...
    if (kFixedBufferSize == type)
    {
        ...
        {
            WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "size paraWEBRTC_ter is out of range");
            return -1;
        }
    }
