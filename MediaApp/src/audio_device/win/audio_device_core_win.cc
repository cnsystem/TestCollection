...
// An impleWEBRTC_ntation of IMediaBuffer, as required for
...
bool AudioDeviceWindowsCore::CoreAudioIsSupported()
{
    ...
    // and creates a new apartWEBRTC_nt for the thread if one is required. The
    ...
    // to the other interfaces in the MMDevice API. The MMDevice API impleWEBRTC_nts
    ...
    // IMWEBRTC_ndpoint          Represents an audio endpoint device.
    //
    ...
    _newMicLevel(0)
{
    ...
    // and deallocate in Terminate() to make the impleWEBRTC_ntation more symWEBRTC_tric.
    CoCreateInstance(
...
WebRtc_Word32 AudioDeviceWindowsCore::Init()
{
    ...
    // Note that, soWEBRTC_ of these will not be able to select by the user.
...
WebRtc_Word32 AudioDeviceWindowsCore::InitPlayout()
{
    ...
    // event driven, the basic buffer manageWEBRTC_nt process, as described previously,
    // is unaltered.
    ...
    if (_ptrAudioBuffer)
    {
        // Update the audio buffer with the selected paraWEBRTC_ters
        _ptrAudioBuffer->SetPlayoutSampleRate(_playSampleRate);
        _ptrAudioBuffer->SetPlayoutChannels((WebRtc_UWord8)_playChannels);
    }
    else
    {
        // We can enter this state during CoreAudioIsSupported() when no AudioDeviceImpleWEBRTC_ntation
...
WebRtc_Word32 AudioDeviceWindowsCore::InitRecordingDMO()
{
    ...
    // Set the DMO output format paraWEBRTC_ters.
    ...
    {
        // Refer to InitRecording() for comWEBRTC_nts.
...
WebRtc_Word32 AudioDeviceWindowsCore::StopPlayout()
{
...

    {
        ...
        // this event might be caught by the new render thread within saWEBRTC_ VoE instance.
...
WebRtc_Word32 AudioDeviceWindowsCore::PlayoutBuffer(AudioDeviceModule::BufferType& type, WebRtc_UWord16& sizeMS) const
{
    ...
    {
        // Use saWEBRTC_ value as for PlayoutDelay
...
DWORD AudioDeviceWindowsCore::DoCaptureThreadPollDMO()
{
    ...
    while (keepRecording)
    {
        ...
        // (The saWEBRTC_ interval used in the Wave impleWEBRTC_ntation.)
        ...
        while (keepRecording)
        {
            ...
            if (!(dwStatus & DMO_OUTPUT_DATA_BUFFERF_INCOMPLETE))
            {
                ...
                // normal case; otherwise it WEBRTC_ans the DMO had more than 10 ms
        ...
        while (keepRecording)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                while (syncBufIndex >= _recBlockSize)
                {
                    ...
                    sndCardRecDelay -= 10;
                }//end of while (syncBufIndex >= _recBlockSize)
...
int AudioDeviceWindowsCore::SetDMOProperties()
{
    ...
    // Relevant paraWEBRTC_ters to leave at default settings:
    ...
    // MFPKEY_WMAAECMA_FEATR_FRAWEBRTC__SIZE - Frame size (0).
...
WebRtc_Word32 AudioDeviceWindowsCore::_EnumerateEndpointDevicesAll(EDataFlow dataFlow) const
{
    ...
    hr = _ptrEnumerator->EnumAudioEndpoints(
                                 dataFlow,            // data-flow direction (input paraWEBRTC_ter)
    ...
    for (ULONG i = 0; i < count; i++)
    {
        ...
        if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_METER)
            // The audio endpoint device supports a hardware peak WEBRTC_ter
