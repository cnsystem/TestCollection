...
void AudioDeviceWindowsWave::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer)
{
    ...
    // inform the AudioBuffer about default settings for this impleWEBRTC_ntation
...
WebRtc_Word32 AudioDeviceWindowsWave::Init()
{
    ...
    const char* threadName = "WEBRTC__AUDIO_module_thread";
...
WebRtc_Word32 AudioDeviceWindowsWave::SpeakerMuteIsAvailable(bool& available)
{
    ...
    if (InitSpeaker() == -1)
    {
        // If we end up here it WEBRTC_ans that the selected speaker has no volume
...
WebRtc_Word32 AudioDeviceWindowsWave::MicrophoneMuteIsAvailable(bool& available)
{
    ...
    if (InitMicrophone() == -1)
    {
        // If we end up here it WEBRTC_ans that the selected microphone has no volume
...
WebRtc_Word32 AudioDeviceWindowsWave::StopRecording()
{
    ...
    // Clean up the preparation perforWEBRTC_d by the waveInPrepareHeader function.
    // Only unprepare header if recording was ever started (and headers are prepared).
    //
    if (wasRecording)
    {
        WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveInUnprepareHeader() will be perforWEBRTC_d");
...
WebRtc_Word32 AudioDeviceWindowsWave::StopPlayout()
{
    ...
    // The waveOutUnprepareHeader function cleans up the preparation perforWEBRTC_d
...
void AudioDeviceWindowsWave::TraceSupportFlags(DWORD dwSupport) const
{
    ...
    if (dwSupport & WAVECAPS_VOLUME)
    {
        ...
        StringCchCat(buf, 256, TEXT("(VOLUWEBRTC_)"));
    }
    if (dwSupport & WAVECAPS_LRVOLUME)
    {
        // supports separate left and right volume control
        StringCchCat(buf, 256, TEXT("(LRVOLUWEBRTC_)"));
    }
...
WebRtc_Word32 AudioDeviceWindowsWave::GetPlayoutBufferDelay(WebRtc_UWord32& writtenSamples, WebRtc_UWord32& playedSamples)
{
    ...
    if (playedDifference > 64000)
    {
        ...
        // sampels. To avoid being fooled by sound cards that soWEBRTC_times produces false
        // output we compare old value minus the new value with a large value. This is
        // neccessary because soWEBRTC_ SC:s produce an output like 153, 198, 175, 230 which
    ...
    // When the exponential WEBRTC_an value of diff(y) goes away from zero soWEBRTC_thing is wrong
    // The exponential formula will accept 1% clock drift but not more
    // The driver error WEBRTC_ans that we will play to little audio and have a high negative clock drift
    ...
    // Check if playout buffer is extreWEBRTC_ly low, or if we haven't been able to
    ...
    if ((playedSamples != 0))
    {
        ...
        {
            diff = y - _dc_y_prev;
            _dc_diff_WEBRTC_an = (990*_dc_diff_WEBRTC_an)/1000 + 10*diff;
        }
        _dc_y_prev = y;
    }
    ...
    if (_dc_diff_WEBRTC_an < -200)
    {
        ...
        // If that does not help, we must modify this scheWEBRTC_ further.
        ...
        {
            _minPlayBufDelay = 100;   // add soWEBRTC_ more safety
...
bool AudioDeviceWindowsWave::ThreadProcess()
{
       ...
       (recDiff < 0))
    {
        ...
        if (_recording)
        {
            ...
            // can soWEBRTC_timed enter a "bad state" after hibernation where the resolution is
            ...
            while ((nRecordedBytes = RecProc(recTime)) > 0)
            {
                ...
                if (maxIter == 0)
                {
                    // If we get this Message ofte, our compensation scheWEBRTC_ is not sufficient.
...
int AudioDeviceWindowsWave::PlayProc(LONGLONG& consumedTime)
{
    ...
    // The threshold can be adaptive or fixed. The adaptive scheWEBRTC_ is updated
...
//  MonitorRecording
//
//  Restart timer if needed (they seem to be WEBRTC_ssed up after a hibernate).
// ----------------------------------------------------------------------------
...
WebRtc_Word32 AudioDeviceWindowsWave::RestartTimerIfNeeded(const WebRtc_UWord32 time)
{
    ...
    if (diffMS > 7)
    {
        ...
        if (_timerFaults > 5 && _timerRestartAttempts < 2)
        {
            ...
            // On soWEBRTC_ machines it helps and the timer starts working as it should again;
            // however, not all machines (we have seen issues on e.g. IBM T60).
            // Therefore, the scheWEBRTC_ below ensures that we do max 2 attempts to restart the timer.
    ...
    {
        // restart timer-check scheWEBRTC_ since we are OK
        _timerFaults = 0;
        _timerRestartAttempts = 0;
    }
